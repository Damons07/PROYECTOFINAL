...
if pagina_seleccionada == "Resumen Ejecutivo":
    ...
    paises = datos_ventas['País'].unique()

    pais_seleccionado = st.sidebar.selectbox("Selecciona un país:", paises)

    # Crear colnua
    datos_ventas['Seleccionado'] = datos_ventas['País'].apply(
        lambda x: 'Seleccionado' if x == pais_seleccionado else 'No Seleccionado')
    st.subheader("Distribución de Ventas")

    fig_ventas_por_pais = px.bar(datos_ventas, x='País', y='Importe venta total',
                                 color='Seleccionado',
                                 title='Ventas por País', labels={'Importe venta total': 'Ventas'},
                                 color_discrete_map={'Seleccionado': 'red', 'No Seleccionado': 'blue'})
    st.plotly_chart(fig_ventas_por_pais)

    datos_pais = datos_ventas[datos_ventas['País'] == pais_seleccionado]
    ...
    fig = px.choropleth(datos_pais,
                        ...
                        hover_name='País',
                        title=f"Mapa de Ventas para {pais_seleccionado}",
